    ...
    <div id="create-collapse" class="accordion-collapse collapse mb-3" wire:ignore.self>
        <div class="accordion-body pt-0">
            <div class="card card-body">
                <div class="row">
                    <div class="col-md-6">
                        <label class="form-label required">Location Code</label>
                    ...
                    <div class="col-md-6 mt-3">
                        ...
                        @if (count($warehouse) > 0)
                            <select class="form-select @error('warehouse_id') is-invalid @enderror"
                                ...
                                @foreach ($warehouse as $w)
                                    <option value="{{ $w->id }}">{{ $w->warehouse_code }} -
                                        {{ $w->warehouse_name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        @else
                            <p>
                                <i>Belum ada Data. Harap buat <a href="{{ route('master.warehouse') }}">Master
                                        Warehouse</a> terlebih dahulu!</i>
                            </p>
                            @error('warehouse_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-6 mt-3">
                        <label class="form-label required">Rak</label>
                        @if (count($rak) > 0)
                            <select class="form-select @error('rak_id') is-invalid @enderror" wire:model="rak_id">
                                <option value="">-- Select --</option>
                                @foreach ($rak as $r)
                                    <option value="{{ $r->id }}">{{ $r->rak_code }} - {{ $r->rak_name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('rak_id')
                        ...
                        @else
                            <p>
                                <i>Belum ada Data. Harap buat <a href="{{ route('master.rak') }}">Master
                                        Rak</a> terlebih dahulu!</i>
                            </p>
                            @error('rak_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <tbody>
                            @foreach ($data as $d)
                                <tr wire:key="{{ $d->id }}">
                                    ...
                                    <td>{{ $d->warehouse->warehouse_code }} - {{ $d->warehouse->warehouse_name }}</td>
                                    <td>{{ $d->rak->rak_code }} - {{ $d->rak->rak_name }}</td>
                                    <td>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Edit Master Location</h5>
                ...
                <div wire:target="updateConfirm" wire:loading.remove>
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6">
                                <label class="form-label required">Location Code</label>
                                <input type="text" class="form-control @error('location_code_') is-invalid @enderror"
                                    wire:model="location_code_">
                                @error('location_code_')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6">
                                <label class="form-label required">Location Name</label>
                                <input type="text" class="form-control @error('location_name_') is-invalid @enderror"
                                    wire:model="location_name_">
                                @error('location_name_')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6 mt-3">
                                <label class="form-label required">Warehouse</label>
                                @if (count($warehouse) > 0)
                                    <select class="form-select @error('warehouse_id_') is-invalid @enderror"
                                        wire:model="warehouse_id_">
                                        @foreach ($warehouse as $w)
                                            <option value="{{ $w->id }}">{{ $w->warehouse_code }} -
                                                {{ $w->warehouse_name }}
                                            </option>
                            ...
                            <div class="col-md-6 mt-3">
                                <label class="form-label required">Rak</label>
                                @if (count($rak) > 0)
                                    <select class="form-select @error('rak_id_') is-invalid @enderror"
                                        wire:model="rak_id_">
                                        @foreach ($rak as $w)
                                            <option value="{{ $w->id }}">{{ $w->rak_code }} -
                                                {{ $w->rak_name }}
                                            </option>
                                        @endforeach
                                    </select>
